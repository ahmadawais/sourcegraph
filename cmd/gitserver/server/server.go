...
func (s *Server) NewRepoStateSyncer(ctx context.Context, interval time.Duration, batchSize, perSecond int) goroutine.BackgroundRoutine {
...
func (s *Server) syncRepoState(gitServerAddrs gitserver.GitserverAddresses, batchSize, perSecond int, fullSync bool) error {
	...
	for {
		...
		for _, repo := range repos {
			...
			addr := addrForRepo(ctx, s.Logger, repo.Name, gitServerAddrs)
			if !hostnameMatch(s.Hostname, addr) {
...
func (s *Server) handleRepoClone(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Error("cannot find a repo in the database", log.String("repo", string(repoName)))
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	_, _, err = s.DB.RepoUpdateJobs().Create(ctx, database.RepoUpdateJobOpts{RepoID: repo.ID})
	if err != nil {
		logger.Warn("error cloning repo", log.String("repo", string(repoName)), log.Error(err))
		resp.Error = err.Error()
	}
...
func (s *Server) search(ctx context.Context, args *protocol.SearchRequest, onMatch func(*protocol.CommitMatch) error) (limitHit bool, err error) {
	...
	if !repoCloned(dir) {
		...
		repo, err := s.DB.Repos().GetByName(ctx, repoName)
		if err != nil {
			s.Logger.Debug("cannot find a repo in the database", log.String("repo", string(repoName)), log.Error(err))
			return false, &gitdomain.RepoNotExistError{
				Repo:            repoName,
	...
	for _, rev := range args.Revisions {
		...
		if rev.RevSpec != "" {
			_ = s.ensureRevision(ctx, repoName, rev.RevSpec, dir)
		} else if rev.RefGlob != "" {
			_ = s.ensureRevision(ctx, repoName, rev.RefGlob, dir)
		}
	}
...
func (s *Server) doClone(ctx context.Context, repo api.RepoName, dir common.GitDir, syncer VCSSyncer, lock *RepositoryLock, remoteURL *vcs.URL, opts CloneOptions) (err error) {
	...
	if err := setRepositoryType(s.RecordingCommandFactory, s.ReposDir, tmp, syncer.Type()); err != nil {
	...
	if err := gitSetAutoGC(s.RecordingCommandFactory, s.ReposDir, tmp); err != nil {
		return err
	}
...
func (s *Server) doBackgroundRepoUpdate(repo api.RepoName, revspec string) error {
	...
	if err := setRepositoryType(s.RecordingCommandFactory, s.ReposDir, dir, syncer.Type()); err != nil {
